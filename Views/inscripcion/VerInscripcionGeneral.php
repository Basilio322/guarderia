...

{
	...
	function Header()
	{
		...
		$this->Ln(10);
		$this->Cell(0, 0, utf8_decode('Instituto Tecnológico "Don Bosquito"'), 0, 0, 'C');
		// Salto de línea
		$this->Ln(10);
	}
...
foreach ($data['inscripcion'] as $row) {
	$pdf->Cell(67, 5, utf8_decode($row['his_Peso1']), 1, 0, 'L');
	$pdf->Cell(56, 5, utf8_decode($row['his_Talla1']), 1, 0, 'L');
...
foreach ($data['inscripcion'] as $row) {
	...
	$pdf->Cell(56, 5, utf8_decode($row['ins_Gestion']), 1, 0, 'L');
